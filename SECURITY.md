...
When a key is sent at 12:00 PM America/New_York, the backend must store the Discord DM message ID and schedule deletion for 12:00 AM America/New_York, twelve hours later. The key must expire server-side independently of message deletion. Handle daylight-saving transitions with the `America/New_York` timezone, and treat a failed Discord deletion as an operational error without making the key valid again.
